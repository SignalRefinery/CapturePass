-- Add split text phone fields to profiles and profile views.
--
-- This keeps text-contact numbers separate from call numbers so public
-- profiles and multi-view pages can render the correct SMS target.

alter table public.profiles
  add column if not exists text_phone text not null default '';

alter table public.profile_views
  add column if not exists text_phone text not null default '';

comment on column public.profiles.text_phone
is 'Optional separate phone number for text-based contact actions and contact downloads.';

comment on column public.profile_views.text_phone
is 'Optional separate phone number for text-based contact actions and contact downloads.';
...
is 'Returns the limited public profile payload for active, visible, approved slug pages, including split phone fields and secondary button mode.';
